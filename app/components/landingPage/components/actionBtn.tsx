...
const Btn: React.FC = forwardRef<HTMLAnchorElement | null>(
  ({ ...props }, ref) => {
    const {
      LINK_NEW_TAB,
      ACTION_DESTINATION,
    ...
    const onClick = () => {
      ...
      else ACTION_DESTINATION && router.push(ACTION_DESTINATION);
    };
    ...
    return (
      <Button
        ...
        onClick={(e: ChangeEvent<HTMLButtonElement>) => {
          if (!editable?.[0])
            if (ACTION_DESTINATION) onClick();
            else alert("your action");
        ...
        (editable?.[0] ? (
          <Stack rowGap={1}>
            <Tooltip
              ...
              title={
                <IconButton
                  onClick={() => setEditDestination(!editDestination)}
                >
                  <EditIcon />
                </IconButton>
              }
              ...
              <Box>
                <Tooltip
                  ...
                  title={
                    <IconButton
                      onClick={() => {
                        if (editActionStatement[0])
                          confirmActionStatement[1](true);
            ...
            >
              ...
              >
                ...
                <RadioGroup
                  ...
                  value={LINK_NEW_TAB}
                  onChange={(e) =>
                    cms?.linkNewTab.setter(Boolean(e.target.value))
                  }
                >
